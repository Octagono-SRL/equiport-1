...
class OctBankReconciliation(models.Model):
    ...
    current_checks1 = fields.Float(string="Cheques en transito", compute='_compute_current_checks1')  # Computado(resta)
    ...
    issued_deposits = fields.Float(string="Depositos emitidos",
                                   compute='_compute_issued_deposits_and_current_checks2', store=True) #se suma
    current_checks2 = fields.Float(string="Cheques emitidos",
                                   compute='_compute_issued_deposits_and_current_checks2', store=True) #se resta
    ...
    total = fields.Float(string=u"Total de conciliación del mes", compute='_compute_total', store=True)
    # total = fields.Float(string=u"Total de conciliación del mes")
    ...
    def _compute_total(self):
        for rec in self:
            print(1)
            rec.total = rec.previous_balance + rec.issued_deposits - rec.current_checks2
    ...
    def generate_reconciliation(self):
        ...
        for line in unvalidated_reconciliations_current:
            ...
            lines.append((0, 0, {'date': line.date,
                                 ...
                                 'account_id': line.account_id.id,
                                 'partner_id': line.partner_id.id,
                                 'debit': abs(line.amount_currency) if line.amount_currency != 0.0 and line.debit > 0.0 else line.debit,
                                 'credit': abs(line.amount_currency) if line.amount_currency != 0.0 and line.credit > 0.0 else line.credit}))
    ...
    def _compute_issued_deposits_and_current_checks2(self):
        for rec in self.filtered('period'):
    ...
    def _compute_current_checks1(self):
        for rec in self:
            rec.current_checks1 = sum(line.credit if not line.bank_reconciliated else 0.0 for line in self.payment_ids)
