...
class ReportPartnerAccount(models.TransientModel):
    ...
    _description = "Modelo para visualizar el estado de cuentas de los clientes"

    name = fields.Char(string='name', compute='_compute_rec_name')
    date_from = fields.Date(string='Desde')
    date_to = fields.Date(string='A la Fecha', default=fields.Date.today())
    ...
    amount_residual = fields.Monetary(string='Total Adeudado', store=True, readonly=True,
                                      compute='_compute_amount')
    ...
    def action_document_print(self):
        if len(self.line_ids) > 0:
            ...
            raise UserError("No hay informacion para procesar el PDF")
...
class ReportPartnerAccountLine(models.TransientModel):
    ...
    _description = "Modelo para visualizar las lineas del estado de cuentas de los clientes"
